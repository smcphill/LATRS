module Entry::TestablesHelper
  ...
  def time_widget(form, symbol)
    opts = {:time => "mixed",
      ...
      :value => Time.new.strftime(FULLTIME).gsub(/ 0(\d\D)/, ' \1')}
  ...
  def testable_datatype_search_column(record, input_name)
    select :record, :datatype, Testable.all(:select => "DISTINCT(datatype)").collect {|t| [t.datatype, t.datatype]}, {:include_blank => as_('- select -')}, input_name
  end
  ...
  def field_input(field, builder)
    ...
    input += builder.hidden_field(:label, :value => field.label) if not field.label.nil?
